...
import { submitStaffTicket } from "./actions";

export default async function StaffHelpdeskPage() {
  ...
  if (email) {
    const [staffRows] = await db.execute("SELECT id FROM staff WHERE email = ?", [email]);
    staffMember = (staffRows as any[])[0];
  }

  // Fetch their previous tickets, filtering by 'STAFF'
  let tickets: any[] = [];
  if (staffMember) {
    const [ticketRows] = await db.execute(
      "SELECT * FROM tickets WHERE reporterId = ? AND reporterType = 'STAFF' ORDER BY createdAt DESC",
      [staffMember.id]
    );
  ...
  return (
    <div className="max-w-5xl space-y-8">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8">
        <h2 className="text-xl font-black text-slate-800">Report an Issue</h2>
        <p className="text-slate-500 text-sm mt-1">Submit technical complaints, software bugs, or general IT inquiries.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <form action={submitStaffTicket} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div>
              ...
              <input type="text" name="title" placeholder="e.g. Finance dashboard won't load" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm" required />
            </div>
